...
def create_higgs_mass_lists(m1, m2, m3, m4):
...
def closest_mass_pairs(mass1, mass2, mass3, mass4):
    '''given a set of pt, eta, phi, mass values,
    returns a list of list of jet pairs in each event based on closest Higgs Mass'''
    all_pairs = []
    for i in range(len(mass1)):
        mass_list = create_higgs_mass_lists(mass1[i], mass2[i], mass3[i], mass4[i])
        first_pair = findMin(mass_list)
        # remove all pair combos that are already in the first pair found
        mass_list = [i for i in mass_list if i[1] not in first_pair and i[2] not in first_pair]
        second_pair = findMin(mass_list)
...
pairs = closest_mass_pairs(mass1, mass2, mass3, mass4)
...
plot_histos(paired_higgs_mass, 1, "Mass of 1 Higgs with paired B Jets based on closest Jet Mass Pair", "Higgs Mass (GeV)", 0, 1000, 100)